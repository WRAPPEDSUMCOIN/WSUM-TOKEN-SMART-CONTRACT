...
contract Factory is OwnableContract {
    ...
    function setMerchantDepositAddress(string depositAddress) external  returns (bool) {
        require(!isEmptyString(depositAddress), "invalid asset deposit address");
    ...
    function confirmBurnRequest(bytes32 requestHash, string txid) external returns (bool) {
        uint nonce;
        Request memory request;
